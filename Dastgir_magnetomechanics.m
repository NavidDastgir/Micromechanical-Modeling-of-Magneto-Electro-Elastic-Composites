...
c11_f2=16.6e10; c12_f2=7.7e10; c13_f2=7.8e10; c33_f2=16.2e10; c44_f2=4.3e10;
c66_f2=(c11_f2 - c12_f2)/2; 
e31_f2=-4.4; e33_f2=18.6; e15_f2=11.6;
k11_f2=112e-10; k33_f2=126e-10; 
mu11_f2=5e-6; mu33_f2=10e-6;    
L_F2 = zeros(12,12);
...
L_F2(8,4) = e15_f2; L_F2(4,8) = e15_f2;
L_F2(7,5) = e15_f2; L_F2(5,7) = e15_f2;
...
c11_f3=28.6e10; c12_f3=17.3e10; c13_f3=17.05e10; c33_f3=26.95e10; c44_f3=4.53e10;
c66_f3=(c11_f3 - c12_f3)/2; 
...
L_F3(11,4) = q15_f3; L_F3(4,11) = q15_f3;
L_F3(10,5) = q15_f3; L_F3(5,10) = q15_f3;
...
s2323 = 0.25;
s1313 = 0.25;
s1212 = (3*c11_mat - c12_mat)/(8*c11_mat);
s1143 = (e31_mat)/(2*c11_mat);
s1153 = (q31_mat)/(2*c11_mat);
s2243 = (e31_mat)/(2*c11_mat);
s2253 = (q31_mat)/(2*c11_mat);
s4141 = 0.5;
s4242 = 0.5;
s5151 = 0.5;
s5252 = 0.5;
...
VF_CoFe = 0:0.02:0.6;
y_C11 = zeros(1, length(VF_CoFe));
y_C12 = zeros(1, length(VF_CoFe));
y_C44 = zeros(1, length(VF_CoFe));
y_e31 = zeros(1, length(VF_CoFe));
y_e15 = zeros(1, length(VF_CoFe));
y_q31 = zeros(1, length(VF_CoFe));
y_q15 = zeros(1, length(VF_CoFe));
for i = 1:length(VF_CoFe)   
    c3 = VF_CoFe(i);       % Piezomagnetic phase volume fraction
    c2 = 0.6 - c3;         % Piezelectric phase volume fraction
    c1 = 0.4;              % (matrix+GNPs) phase volume fraction
...
% Report magneto-electro-elastic properties as function of piezomagnetic
% volume fraction
y1 = [VF_CoFe(:), y_C11(:)/1e9];
y2 = [VF_CoFe(:), y_C12(:)/1e9];
y3 = [VF_CoFe(:), y_C44(:)/1e9];
y4 = [VF_CoFe(:), y_e15(:)];
y5 = [VF_CoFe(:), y_e31(:)];
y6 = [VF_CoFe(:), y_q15(:)];
y7 = [VF_CoFe(:), y_q31(:)];
...
subplot(3,1,1); plot(VF_CoFe*100, y_C11/1e9, 'b-o', 'LineWidth', 1.5);
ylabel('C_{11} (GPa)'); grid on; title('Longitudinal Stiffness C_{11}');

subplot(3,1,2); plot(VF_CoFe*100, y_C12/1e9, 'r-s', 'LineWidth', 1.5);
ylabel('C_{12} (GPa)'); grid on; title('Transverse Stiffness C_{12}'); 

subplot(3,1,3); plot(VF_CoFe*100, y_C44/1e9, 'k-^', 'LineWidth', 1.5);
ylabel('C_{44} (GPa)'); xlabel('CoFe_2O_4 Volume Fraction (%)'); grid on; title('Shear Stiffness C_{44}');

% Figure 2: Piezoelectric Coefficients (e)
figure('Name', 'Piezoelectric Coefficients', 'Color', 'w');
subplot(2,1,1); plot(VF_CoFe*100, y_e31, 'm-o', 'LineWidth', 1.5);
ylabel('e_{31} (C/m^2)'); grid on; title('Piezoelectric Coefficient e_{31}');

subplot(2,1,2); plot(VF_CoFe*100, y_e15, 'c-s', 'LineWidth', 1.5);
ylabel('e_{15} (C/m^2)'); xlabel('CoFe_2O_4 Volume Fraction (%)'); grid on; title('Piezoelectric Coefficient e_{15}');

% Figure 3: Piezomagnetic Coefficients (q)
figure('Name', 'Piezomagnetic Coefficients', 'Color', 'w');
subplot(2,1,1); plot(VF_CoFe*100, y_q31, 'g-o', 'LineWidth', 1.5);
ylabel('q_{31} (N/Am)'); grid on; title('Piezomagnetic Coefficient q_{31}');

subplot(2,1,2); plot(VF_CoFe*100, y_q15, 'y-s', 'LineWidth', 1.5);
ylabel('q_{15} (N/Am)'); xlabel('CoFe_2O_4 Volume Fraction (%)'); grid on; title('Piezomagnetic Coefficient q_{15}');
